...
//                    " y se encuentra en la fila " + fila + " y columna  " + col + "y se repite " +repeat+ " veces");
...
//                        int par = matriz[i][j];
//                        System.out.println("El número >> " +par+ " se encuentra en la fila " +i+ ", columna " +j);
//                    }
//                }
//            }
...
//            for (int i = 0; i < fila ; i++) {
//                for (int j = 0; j < col; j++) {
//                    if (AlgoritmosArreglos.esPrimo(matriz[i][j])) { // Llamada a método estático "esPrimo" que verifica si un número es primo o no
...
//            vecSum[i] = sum; // Se almacenan las sumas en un vector para saber cual es el mayor
//            sum = 0; // Se reinicia el valor de suma para almacenar el nuevo valor
//            cont++; // contador de filas
//        }
//
//        for (int i = 0; i < vecSum.length; i++) {
//            if (vecSum[i] > vecSum[sumMayor] ) { // Condición si encuentra algún número mayor que el anterior
...
//                }
//            }
//            System.out.println("mayor fila " +(i+1)+ " es: " +matriz[sumMayor][c]);
//            sum = sum + matriz[sumMayor][c];
//            pro = sum/matriz.length;
//            System.out.println("El promedio de los elementos mayores de cada fila es " +pro);
//            sumMayor = 0;
//            c = 0;
//            cont++; // contador de filas
//        }
...
//        for (int i = 0; i < fila; i++) {
//            for (int j = 0; j < col; j++) {
//                if (AlgoritmosArreglos.esPrimo(matriz[i][j]) && matriz[i][j] % 3 == 0) { // Llamada a método estático "esPrimo" que verifica si un número es primo o no
//                    cont++;
//                }
//            }
//        }
...
//                    par = true;
//                    if (aux == 0) { // Variable auxiliar que asigna una sola vez el primer valor par
//                        menorf = i; // Se asignan las posiciones pares (Fila)
//                        menorC = j; // (Columna)
//                    }
//                    aux = 1;
//                    if (matriz[i][j] < matriz[menorf][menorC]) { // Se compara si es menor EJ (2 < 2) (1 < 2)
//                        menorf = i; // entra --> se pasan las posiciones y quedan las del menor (fila)
//                        menorC = j; // (columna)
//                    }
//                }
//            }
...
//                + "es el mayor y empieza en 4, en la fila " + mayorF + " columna " + mayorC);
